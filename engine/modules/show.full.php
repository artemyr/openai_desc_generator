...
if( !defined('DATALIFEENGINE') ) {
	...
	if ( $row['id'] AND  $perm ) {
					...
					if( $config['allow_alt_url'] ) {
						if( $config['seo_type'] == 1 OR $config['seo_type'] == 2 ) {
							if( intval( $row_link['category'] ) and $config['seo_type'] == 2 ) {
								//$link = $config['http_home_url'] . get_url( intval( $row_link['category'] ) ) . "/" . $row_link['id'] . "-" . $row_link['alt_name'] . "/";
								$link = $config['http_home_url'] . 'game' . "/" . $row_link['id'] . "-" . $row_link['alt_name'] . "/";
							} else {
								$link = $config['http_home_url'] . $row_link['id'] . "-" . $row_link['alt_name'] . "/";
							}
						} else {
							$link = $config['http_home_url'] . date( 'Y/m/d/', strtotime( $row_link['date'] ) ) . $row_link['alt_name'] . "/";
						}
					} else {
					...
					for($j = 1; $j <= $anzahl_seiten; $j ++) {
						...
						if( $j != $news_page ) {
							...
							if( $config['allow_alt_url'] ) {
								...
								if($j != 1) $canonical = $short_link . "page," . $j . "," . $row['alt_name'] . "/";
		...
		$i_razrab = '';
		$xfieldsart = explode("||",$row['xfields']);
		foreach ($xfieldsart as $key => $field){
			$arField = explode("|",$field);
			if($arField[0] == 'i_razrab') $i_razrab = ' от '.$arField[1];
		}

		$metatags['title'] = "Игра " . $row['title'] . $i_razrab . " - Играть Онлайн Бесплатно";

		if( $row['keywords'] == '' AND $row['descr'] == '' AND $config['create_metatags'] ) {
			create_keywords( $row['full_story'] );
		} else {
			$metatags['keywords'] = $row['keywords'];
			if( $row['descr'] ) $metatags['description'] = 'Смотрите онлайн видео '.$row['title'].$i_razrab.' бесплатно, без смс и регистрации.'; else $metatags['description'] = $row['title'];
		}
		...
		if( $config['related_news'] AND $view_template != "print") {
			...
			if( $related_buffer === false ) {
				...
				if ( $row['related_ids'] ) {
					...
					$db->query( "SELECT id, date, short_story, xfields, title, category, alt_name, MATCH (title, short_story, full_story, xfields) AGAINST ('{$body}') as score FROM " . PREFIX . "_post {$join_category}WHERE {$where_category}{$allowed_cats}{$not_allowed_cats}MATCH (title, short_story, full_story, xfields) AGAINST ('{$body}') AND id != " . $row['id'] . " AND approve=1 AND category NOT IN (105)" . $where_date . " ORDER BY score DESC LIMIT " . $config['related_number'] );
